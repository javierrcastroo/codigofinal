...
def detect_table_schema(curs, table: str) -> Dict[str, str]:
    ...
    """
    # Impala DESCRIBE returns rows with first col = column name
    curs.execute("DESCRIBE {}.{}".format(DB_NAME, table))
    cols = [(r[0] or "").strip().lower() for r in curs.fetchall() if r and (r[0] or "").strip()]
    cols_set = set(cols)

    if "partnumber" in cols_set:
        pn_col = "partnumber"
    elif "pn" in cols_set:
        pn_col = "pn"
    else:
...
def get_top_partnumber_all_years_with_cursor(curs, table: str, cols: Dict[str, str]) -> Tuple[int, int]:
    _, res = impala_exec(curs, sql_get_top_partnumber_all_years(table, cols))
    if not res:
...
def get_top_partnumber_months_filter_with_cursor(
    ...
    _, res = impala_exec(curs, sql_get_top_partnumber_months_filter(table, cols))
    if not res:
        return None
    return int(res[0][0]), int(res[0][1])
...
def get_top_partnumber_all_years(table: str) -> Tuple[int, int]:
    curs, conn = impala_on()
    impala_apply_session_settings(curs)
    cols = detect_table_schema(curs, table)
    out = get_top_partnumber_all_years_with_cursor(curs, table, cols)
